...
const Layout = () => {
  ...
  const tabs = [
    { name: 'Dashboard', path: '/' },
    { name: 'Maintenance', path: '/requests/new' },
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <nav style={{ backgroundColor: 'white', borderBottom: '1px solid #e5e7eb', padding: '0 24px' }}>
        <div style={{ display: 'flex', gap: '32px', height: '60px', alignItems: 'center' }}>
          <div style={{ fontWeight: 'bold', fontSize: '20px', marginRight: '20px' }}>GearGuard</div>
          {tabs.map(tab => (
            <Link 
              key={tab.name} 
              to={tab.path}
              style={{
                textDecoration: 'none',
                color: location.pathname === tab.path ? '#2563eb' : '#4b5563',
